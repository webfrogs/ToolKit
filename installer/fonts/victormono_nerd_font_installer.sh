...
GITHUB_ZIP_FILE_URL="$GITHUB_RELEASE_URL/download/$GITHUB_RELEASE_TAG/VictorMono.zip"

GITHUB_ZIP_FILENAME="victormono.zip"
downloadFile "$GITHUB_ZIP_FILE_URL" "$(pwd)/${GITHUB_ZIP_FILENAME}"

mkdir VictorMono
unzip ${GITHUB_ZIP_FILENAME} -d VictorMono
cp VictorMono/* "$fontDir"
